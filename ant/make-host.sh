...
#================================================================#
	...
	usage="usage: $0 -p <package> [-s] [-i thumb | arm] [-a all | armeabi | armeabi-v7a] [-l appPlatform] [--use-fmod true | false] [--use-untz true | false] [--disable-adcolony] [--disable-billing] [--disable-chartboost] [--disable-crittercism] [--disable-facebook] [--disable-push] [--disable-tapjoy] [--disable-sphinx]"
	skip_build="false"
	package_name=
	arm_mode="arm"
	...
	while [ $# -gt 0 ];	do
	    case "$1" in
			...
			--disable-sphinx)  sphinx_flags="--disable-sphinx";;
			-*)
		    	echo >&2 \
		    		$usage
		    	exit 1;;
	...
	if [ x"$skip_build" != xtrue ]; then
		pushd libmoai > /dev/null
			bash build.sh -i $arm_mode -a $arm_arch -l $app_platform --use-fmod $use_fmod --use-untz $use_untz $adcolony_flags $billing_flags $chartboost_flags $crittercism_flags $facebook_flags $push_flags $tapjoy_flags $sphinx_flags
		popd > /dev/null
	fi
	...
	if [ x"$sphinx_flags" == x ]; then
		required_libs="$required_libs \"sphinx\""
	fi
